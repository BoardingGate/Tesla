...
//=======================================================        
    ...
    let isInitialMapLoadAnimationActive= true;
...
// ===================================================================
// NOMBRE: checkAndMarkWaypointsAsVisited (VERSIÓN FINAL Y REFINADA)
...
function saveSettings() {
    ...
    localStorage.setItem(PERFORMANCE_RATIO_GRAPH_KEY, performanceRatios.performanceGraph);
...
function showNavigationMapHelpModal() {
    ...
    // --- INICIO DE LA LÓGICA DE ACTUALIZACIÓN DE INTERVALOS ---
    ...
    const setupRatioSlider = (config) => {
        ...
        slider.addEventListener('input', () => {
            ...
            updateAllIntervalDisplays(); // Actualiza su propio texto y el de los demás
        });
    };
    ...
    // --- FIN DE LA LÓGICA DE ACTUALIZACIÓN DE INTERVALOS ---
    ...
    // Lógica para sliders de Saturación y Contraste (sin cambios)
    ...
    contrastSlider.value = getComputedStyle(document.documentElement).getPropertyValue('--map-contrast').trim() || '1.2';
    updateLabels();
...
//          no serializables como los marcadores del mapa para evitar errores de referencia circular.
// ===================================================================
function sanitizeWaypointsForStorage(waypoints) {
    if (!Array.isArray(waypoints)) return [];
    return waypoints.map(wp => {
        if (!wp) return null;
        // Se crea un objeto nuevo solo con las propiedades seguras para JSON.stringify
        return {
            id: wp.id,
            lat: wp.lat,
            lng: wp.lng,
    ...
    }).filter(Boolean); // Elimina cualquier posible nulo
}
...
// ===================================================================
// NOMBRE: applyMainUISettings (NUEVA FUNCIÓN)
// RESUMEN: Aplica todas las configuraciones visuales de la interfaz principal (toggles, filtros, etc.).
//          Se usa tanto en la carga normal como en la recuperación para mantener la consistencia.
// ===================================================================
...
// ===================================================================
// NOMBRE:  (VERSIÓN OPTIMIZADA Y CORREGIDA PARA MAPBOX)
...
// ===================================================================
// NOMBRE: startDeviationChecker (VERSIÓN OPTIMIZADA)
// RESUMEN: Inicia un temporizador que comprueba el desvío de ruta de forma eficiente,
//          analizando solo un pequeño segmento de la ruta alrededor de la posición actual.
// ===================================================================
function startDeviationChecker() {
    stopDeviationChecker();
    console.log("[DESVÍO] Iniciando comprobador de desvío optimizado (cada 5 segundos).");
    
    deviationCheckIntervalId = setInterval(() => {
        // Condiciones para no ejecutar la comprobación (sin cambios)
        ...
        // Comprobación de que la ruta y los datos necesarios existen
        ...
        try {
            ...
            // 1. Construir la "mini-ruta" o "ventana de comprobación"
            const searchWindowCoords = [];
            const stepsToInclude = 5; // Revisamos el paso actual y los 4 siguientes
            ...
            for (let i = 0; i < stepsToInclude && legIdx < route.legs.length; i++) {
                ...
                if (stepIdx < currentLeg.steps.length) {
                    ...
                    if (currentStep.geometry && currentStep.geometry.coordinates) {
                        // Añadimos las coordenadas del paso, evitando duplicar el primer punto si no es el primer paso
                    ...
                    legIdx++;
                    stepIdx = 0;
                    // Saltamos el paso si pasamos a la siguiente pierna para evitar huecos en la geometría
                    i--; 
                }
            }

            // Si no tenemos suficientes coordenadas, la comprobación no es fiable, así que salimos.
            if (searchWindowCoords.length < 2) {
                deviationStartTime = 0; // Reseteamos el contador si no podemos comprobar
                return;
            }

            // 2. Crear la línea de la ventana y calcular la distancia
            const routeWindowLineString = turf.lineString(searchWindowCoords);
            const distanceToRoute = turf.pointToLineDistance(carPoint, routeWindowLineString, { units: 'meters' });
            // --- FIN DE LA OPTIMIZACIÓN ---

            // 3. La lógica de detección de desvío es exactamente la misma que antes
            ...
            // En caso de error en Turf (muy improbable con geometrías pequeñas),
            // se mantiene la lógica de seguridad original.
            console.error("[DESVÍO] Error en el cálculo de distancia optimizado:", e);
            if (deviationStartTime === 0) {
                deviationStartTime = Date.now();
            } else if ((Date.now() - deviationStartTime) / 1000 > DEVIATION_MIN_TIME_SECONDS) {
                showDeviationModal();
...
async function toggleNavigationState(isRecalculationStop = false) {
    ...
    if (isNavigating) {
        ...
        if (hasEnoughWaypointsForRoute && hasRouteData) {
            ...
            // --- INICIO MODIFICACIÓN RECUPERACIÓN (CORRECCIÓN FINAL V5) ---
            let routeForStorage;
            ...
            if (currentRouteNameForSaving && currentRouteNameForSaving.startsWith("UBICACIÓN: ") && finalWaypoint) {
                routeForStorage = {
                    ...
                    waypoints: sanitizeWaypointsForStorage([finalWaypoint]) // Sanitiza el waypoint único
                };
            } else {
                routeForStorage = { 
                    name: currentRouteNameForSaving || suggestedName, 
                    waypoints: sanitizeWaypointsForStorage(waypointsForRecents) // Sanitiza todos los waypoints
                };
            }
            addRouteToRecents(routeForStorage); // addRouteToRecents ahora recibe un objeto ya sanitizado

            // Usamos el objeto ya sanitizado para la bandera de recuperación, eliminando el error.
            const recoveryData = { isActive: true, route: routeForStorage };
            localStorage.setItem('boardinggate_nav_recovery_flag', JSON.stringify(recoveryData));
            // --- FIN MODIFICACIÓN RECUPERACIÓN (CORRECCIÓN FINAL V5) ---
...
// RESUMEN: Incluye la variable faltante y la lógica de opacidad para
//          una aparición perfecta y sin parpadeos.
// ===================================================================
function showIntersectionPreviewMap(step, route, isSimulationContextOrClick = false, timerDurationMs = null) {
    ...
    previewWindow.classList.add('rendering'); // 1. Hacer invisible
    previewWindow.classList.remove('hidden'); // 2. Poner en el layout
            ...
            requestAnimationFrame(() => {
                if (intersectionPreviewMap && document.body.contains(mapContainerEl)) {
                    ...
                    intersectionPreviewMap.once('idle', () => {
                        previewWindow.classList.remove('rendering');
                    });
                }
            });
        });
...
// ===================================================================
// NOMBRE: clearRecoveryFlag (NUEVA FUNCIÓN)
...
function addSlideButtonListeners(slideElement, reminder) {
  ...
  // ===================================================================
// NOMBRE: precalculateRouteStepDistances (NUEVA FUNCIÓN AUXILIAR)
// RESUMEN: Añade la distancia acumulada a cada paso de la ruta para optimizar cálculos posteriores.
// ===================================================================
function precalculateRouteStepDistances(routeData) {
    if (!routeData || !routeData.routes || !routeData.routes[0]) {
        ...
        // Aseguramos que no queden restos de un pre-cálculo anterior fallido
        if(routeData && routeData.routes && routeData.routes[0]) {
            delete routeData.routes[0].totalPrecalculatedDistance;
        }
        window.optimizationHasFailed = true; // Marcamos para que el fallback se active
        return;
    }
    ...
    try {
            ...
            route.legs.forEach(leg => {
                if (leg.steps) {
                    leg.steps.forEach(step => {
                        // Almacenamos la distancia acumulada AL INICIO de este paso
                        step.cumulativeDistance = cumulativeDistance;
                        // Actualizamos la distancia acumulada para el siguiente paso
                        cumulativeDistance += (step.distance || 0);
                    });
                }
            });
        }
        // Añadimos una propiedad final para verificar fácilmente si la ruta fue procesada
        ...
        // Si hay un error, nos aseguramos de que no queden datos parciales
        if(routeData && routeData.routes && routeData.routes[0]) {
            delete routeData.routes[0].totalPrecalculatedDistance;
        }
...
// ===================================================================
// NOMBRE: recalculateAndDrawRoute (VERSIÓN CON PRE-CÁLCULO INTEGRADO)
// RESUMEN: Lógica de recálculo que ahora, tras obtener una nueva ruta, la pre-procesa.
// ===================================================================
async function recalculateAndDrawRoute(preserveAccumulatedStats = false, isUserAction = false) {
    ...
    while (attempt <= MAX_RETRIES) {
        ...
        try {
            ...
            // <<< ¡CAMBIO CLAVE! Pre-calculamos las distancias de la nueva ruta.
            precalculateRouteStepDistances(newRouteData);
...
// ===================================================================
// NOMBRE: handleAutoZoom (MODIFICADA - SOLO CALCULA, NO EJECUTA)
...
function handleAutoZoom() {
    ...
    else return null; // Si la velocidad es muy alta, no hacemos nada.

    if (Math.abs(navigationMapInstance.getZoom() - targetZoom) > 0.1) {
        return targetZoom;
    }

    return null; // No se necesita cambio de zoom.
}    
...
// ===================================================================
// NOMBRE: updateInitialUserPosition (VERSIÓN FINAL CON LÓGICA UNIFICADA)
// RESUMEN: Orquestador principal que se ejecuta en cada tick del GPS (real o simulado).
//          Incrementa el contador de ticks y distribuye las tareas según ratios.
// ===================================================================
function updateInitialUserPosition(coords, mapInstanceToUse) {
    if (!mapInstanceToUse || !coords || typeof coords.latitude !== 'number' || typeof coords.longitude !== 'number' || isNaN(coords.latitude) || isNaN(coords.longitude)) {
        console.warn("[GPS] Datos de coordenadas inválidos recibidos. Omitiendo actualización.");
        return;
    }
    ...
    if (gpsTickCounter % performanceRatios.routeProgress === 0) {
        if (isNavigating) {
            processRouteProgress(coords, mapInstanceToUse); // Esta función ahora se encarga de llamar a las demás
        }
    }

    // --- NIVEL 3: FRECUENCIA MEDIA (Controlado por Ratio) ---
    if (gpsTickCounter % performanceRatios.trafficAlerts === 0) {
        checkAlertsProximity();
    }
    
    if (gpsTickCounter % performanceRatios.tripStats === 0) {
        updateTripStatistics(coords);
    }

    // --- NIVEL 4: BAJA FRECUENCIA (Controlado por Ratio) ---
    if (gpsTickCounter % performanceRatios.performanceGraph === 0) {
        plotIntervalData();
    }
}
...
// RESUMEN: Gestiona exclusivamente la posición y rotación del marcador del vehículo.
...
// ===================================================================
// NOMBRE: updateMapCamera (NUEVA FUNCIÓN AUXILIAR)
// RESUMEN: Gestiona exclusivamente el movimiento de la cámara del mapa.
function updateMapCamera(coords, mapInstanceToUse) {
    const shouldFollow = (shouldCenterOnUser || navigationFollowUser);
    if (!shouldFollow || isRecalculatingRoute || !mapInstanceToUse) {
        return;
    }
    
    const lngLat = new mapboxgl.LngLat(coords.longitude, coords.latitude);

    const maneuverCameraAdjustments = handleProgressiveManeuverZoom(
    ...
    let targetZoom = maneuverCameraAdjustments ? maneuverCameraAdjustments.zoom : null;
    let targetPitch = maneuverCameraAdjustments ? maneuverCameraAdjustments.pitch : null;

    if (targetZoom === null) {
        targetZoom = handleAutoZoom();
    }
    
    if (targetZoom !== null && (typeof targetZoom !== 'number' || isNaN(targetZoom))) {
        console.error(`[VALIDACIÓN] Se ha detectado un valor de zoom inválido (${targetZoom}). Se anulará el cambio de zoom en este ciclo para prevenir un error.`);
        targetZoom = null;
    }
    ...
    const mapHeight = mapInstanceToUse.getCanvas().clientHeight;
    const mapWidth = mapInstanceToUse.getCanvas().clientWidth;
    viewOptions.padding = {
        top: mapOffsetY > 0 ? mapHeight * Math.abs(mapOffsetY / 100) : 0,
        bottom: mapOffsetY < 0 ? mapHeight * Math.abs(mapOffsetY / 100) : 0,
        left: mapOffsetX > 0 ? mapWidth * Math.abs(mapOffsetX / 100) : 0,
        right: mapOffsetX < 0 ? mapWidth * Math.abs(mapOffsetX / 100) : 0
    };
    if (mapViewMode === 'heading' || mapViewMode === 'perspective') {
        ...
        if(validatedHeading !== null) {
            viewOptions.bearing = validatedHeading; 
        }
    }

    if (targetPitch !== null && typeof targetPitch === 'number' && !isNaN(targetPitch)) {
        viewOptions.pitch = targetPitch;
    } else {
        viewOptions.pitch = (mapViewMode === 'perspective') ? mapPitchValue : 0;
    }
    ...
    mapInstanceToUse.easeTo(viewOptions);
}

// ===================================================================
// NOMBRE: updateTripStatistics (NUEVA FUNCIÓN AUXILIAR)
            ...
            SPEED_MILESTONES.forEach(milestone => {
                if (window.currentSpeedKmh > milestone && milestone > lastSpeedMilestone) {
                    ...
                    const newMilestoneFeature = {
                        ...
                        properties: { type: 'milestone', speedText: String(milestone) }
                    };
                    speedMarkerFeatures.push(newMilestoneFeature);
                    if (navigationMapInstance && navigationMapInstance.getSource('speed-markers-source')) {
                        navigationMapInstance.getSource('speed-markers-source').setData({ type: 'FeatureCollection', features: speedMarkerFeatures });
                    }
                }
            });
        }
...
// ===================================================================
// NOMBRE: processRouteProgress (NUEVA FUNCIÓN AUXILIAR - CORREGIDA)
// RESUMEN: Lógica de progreso en ruta que CALCULA y LUEGO LLAMA a las funciones de UI.
function processRouteProgress(coords, mapInstanceToUse) {
    if(!isNavigating || !navigationCurrentRouteData || !navigationCurrentRouteData.routes[0] || !coords) return;

    const route = navigationCurrentRouteData.routes[0];
    let carDistTurf = 0;

    try {
        const carPoint = turf.point([coords.longitude, coords.latitude]);
        const searchWindowCoords = [];
        const stepsToIncludeInWindow = 5;
        let legIdx = currentLegIndexNav;
        let stepIdx = currentStepIndexNav;

        for (let i = 0; i < stepsToIncludeInWindow && legIdx < route.legs.length; i++) {
            const currentLeg = route.legs[legIdx];
            if (stepIdx < currentLeg.steps.length) {
                const currentStep = currentLeg.steps[stepIdx];
                if (currentStep.geometry && currentStep.geometry.coordinates) {
        ...
        if (searchWindowCoords.length < 2) {
             throw new Error("Ventana de búsqueda insuficiente. Usando fallback.");
        }

        const routeWindowLineString = turf.lineString(searchWindowCoords);
        const nearestPointOnWindow = turf.nearestPointOnLine(routeWindowLineString, carPoint, { units: 'meters' });
        
        const bestStepInWindow = route.legs[currentLegIndexNav].steps[currentStepIndexNav];
        const cumulativeDistance = bestStepInWindow.cumulativeDistance || 0;
        
        const startOfWindowPoint = turf.point(routeWindowLineString.geometry.coordinates[0]);
        const sliceToPoint = turf.lineSlice(startOfWindowPoint, nearestPointOnWindow, routeWindowLineString);
        const distanceAlongWindow = turf.length(sliceToPoint, { units: 'meters' });
        
        carDistTurf = cumulativeDistance + distanceAlongWindow;

    } catch (e) {
        if (!window.optimizationHasFailed) {
            console.warn("[OPTIMIZATION FALLBACK] Error en búsqueda por ventana. Usando método original.", e);
            window.optimizationHasFailed = true;
        }
        try {
            const carPointTurf = turf.point([coords.longitude, coords.latitude]);
            const routeGeoJSON = mapInstanceToUse.getSource('route')._data;
        ...
        } catch (turfError) { /* Silenciar error de fallback */ }
    }
    ...
    renderWaypointMarkers(); // Necesario para actualizar el color de los visitados
...
// ===================================================================
// NOMBRE: startGpsWatching (VERSIÓN FINAL Y DEFINITIVA - CORREGIDA)
// RESUMEN: Inicia el seguimiento del GPS. Ahora SÓLO se encarga de llamar
//          al orquestador y actualizar el monitor visual de frecuencia.
// ===================================================================
...
// ===================================================================
// NOMBRE: processSimulatedGpsPosition (VERSIÓN FINAL Y CORRECTA)
...
// ===================================================================
// NOMBRE: getGpsWithRetries (NUEVA FUNCIÓN EXTRAÍDA Y CORREGIDA)
...
// ===================================================================
// NOMBRE: getInitialGpsPositionForMap (NUEVA FUNCIÓN GESTORA)
...
// ===================================================================
// NOMBRE: checkForAndExecuteCrashRecovery (MODIFICADA)
// RESUMEN: Llama a openNavigationMap con el nuevo parámetro isRecovery = true.
// ===================================================================
async function checkForAndExecuteCrashRecovery() {
    const recoveryFlagJSON = localStorage.getItem('boardinggate_nav_recovery_flag');
    if (!recoveryFlagJSON) {
        return false; // No hay nada que recuperar, carga normal.
    }

    try {
        const recoveryData = JSON.parse(recoveryFlagJSON);
        if (recoveryData && recoveryData.isActive && recoveryData.route) {
            showToast("Recuperando sesión de navegación anterior...", "info", 5000);
            ...
            try {
                ...
                await loadRouteFromObject(recoveryData.route, true, true);

                return true; 
            
            } catch (recoveryError) {
                console.error("Fallo durante la ejecución de la recuperación automática:", recoveryError);
        ...
        console.error("Error al procesar los datos de recuperación. Limpiando bandera.", e);
        clearRecoveryFlag();
        return false;
    }
...
async function loadRouteFromObject(routeObject, autoStartNavigation = false, forceImmediateStart = false) {
    ...
    if (waypointsFromLoadedRoute.length === 1) {
        // Caso: Se está cargando un destino único (1 waypoint).
        if (startLocation) {
            const startPoint = {
                id: Date.now() - 1,
                lat: startLocation.latitude,
                lng: startLocation.longitude,
                type: 'S', marker: null, label: 'S', address: 'Posición Actual (GPS)',
            ...
            return; // Abortamos la carga si no hay punto de partida.
        }
    } else {
        // Caso: Se está cargando una ruta completa (2 o más waypoints).
        if (startLocation) {
            const startPoint = {
                id: Date.now() - 1,
                lat: startLocation.latitude,
    ...
    if (navigationWaypoints.filter(wp => wp && !wp.isInternal).length >= 2) {
        return new Promise((resolve, reject) => {
            navigationMapInstance.once('idle', async () => {
                let recalculationSuccess = false;
                let attempts = 0;
                const MAX_RECALC_ATTEMPTS = 5;

                while (!recalculationSuccess && attempts < MAX_RECALC_ATTEMPTS) {
                    attempts++;
                    try {
                        await recalculateAndDrawRoute(false);
                        recalculationSuccess = true;
                    } catch (error) {
                        console.error(`Intento de recálculo #${attempts} falló:`, error);
                        if (attempts < MAX_RECALC_ATTEMPTS) {
                            showToast(`Fallo al trazar la ruta (${attempts}/${MAX_RECALC_ATTEMPTS}). Reintentando...`, "warning");
                            await new Promise(resolve => setTimeout(resolve, 3000));
                        }
                    }
                }
                ...
                if (!recalculationSuccess) {
                    showToast("No se pudo trazar la ruta. Comprueba la conexión. Inicia la navegación manualmente cuando sea posible.", "error", 8000);
                    ...
                    updateStartNavigationButtonState();
                    updateNavigationButtonColor();
                    reject(new Error("Recalculation failed after all retries"));
                    return;
                }
...
function stopFreeDrive() {
    ...
    // --- INICIO MODIFICACIÓN RECUPERACIÓN ---
    clearRecoveryFlag();
    // --- FIN MODIFICACIÓN RECUPERACIÓN ---
...
// Esta función NO cambia, es la misma de la respuesta anterior.
function initializeConfigModalListeners() {
    ...
    const monitorResourcesCheckbox = document.getElementById('config-monitor-resources');
    if (monitorResourcesCheckbox && !monitorResourcesCheckbox.dataset.listenerAttached) {
        monitorResourcesCheckbox.addEventListener('change', () => {
            if (monitorResourcesCheckbox.checked) {
        ...
        monitorResourcesCheckbox.dataset.listenerAttached = "true";
    }
}    
...
function addRouteToRecents(routeObject) {
    ...
    // --- INICIO MODIFICACIÓN RECUPERACIÓN (CORRECCIÓN FINAL V7) ---
    // Lógica inteligente para guardar solo el destino en recientes si es una ruta simple.
    let simplifiedRoute;
    ...
    // --- FIN MODIFICACIÓN RECUPERACIÓN (CORRECCIÓN FINAL V7) ---
...
async function saveCurrentRoute() {
    ...
    const onSaveCallback = (confirmedName) => {
        // --- INICIO MODIFICACIÓN RECUPERACIÓN (CORRECCIÓN FINAL V7) ---
        // Se restaura la lógica original: siempre se guardan TODOS los waypoints de la ruta activa en favoritos.
...
function showLoadRouteModal() {
    ...
    const displayRoutes = () => {
        ...
        // --- INICIO MODIFICACIÓN RECUPERACIÓN (CORRECCIÓN FINAL V7) ---
        // Lógica corregida para clasificar rutas simples en "Destinos".
        ...
        // --- INICIO MODIFICACIÓN RECUPERACIÓN (CORRECCIÓN FINAL V7) ---
        // Lógica corregida para clasificar rutas multietapa en "Rutas".
...
function closeNavigationMap(force = false) {
    ...
    // --- INICIO MODIFICACIÓN RECUPERACIÓN (CORRECCIÓN FINAL V4) ---
    // Si la app se cargó directamente en el mapa, debemos forzar la carga y renderizado de la UI.
    ...
    // --- FIN MODIFICACIÓN RECUPERACIÓN (CORRECCIÓN FINAL V4) ---
...
function startResourceMonitor() {
    ...
    resourceMonitorInterval = setInterval(() => {
        ...
        const buttonHtml = `<button onclick="document.getElementById('config-monitor-resources').click()" style="width:100%; margin-top:8px; padding:4px 8px; font-size:0.9rem; font-weight:bold; background-color:#dc3545; color:white; border:none; border-radius:4px; cursor:pointer;">Cancelar</button>`;

        // Usamos la función showToast existente, pero le pasamos el HTML completo
